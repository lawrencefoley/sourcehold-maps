...
class Breakpoints(object):
    ...
    LANDING_POINT_DRAG_DROP_PATH = ("0x4013C5", "landing_point_drag_drop_path")
    LANDING_POINT_DRAG_DROP_PNGPATH = ("0x40143B", "landing_point_drag_drop_pngpath")
    CONVERT_FILE_START = ("0x4026C0", "convert_file_start")
    CONVERT_FILE_END = ("0x401443", "convert_file_end")
    ...
    @staticmethod
    def breakpoints():
        return [Breakpoints.__dict__[key] for key in Breakpoints.__dict__ if key.isupper()]
...
debugger = D.NetworkedDebugger("C:\\Users\\Frank\\Downloads\\smpnger11\\Stronghold Map PNGer.exe")
...
class A1(Testable):
    ...
    def initialize(self):
        debugger.go_to_next_breakpoint(Breakpoints.ARRAY_TRANSLATION_INIT)
        ...
        debugger.go_to_next_breakpoint(Breakpoints.ARRAY_TRANSLATION_INIT_END)
...
def setup_debugger():
    ...
    debugger.delete_breakpoints(Breakpoints.breakpoints())
    debugger.set_breakpoints([INIT_BREAKPOINT])
    debugger.go_to_next_breakpoint(INIT_BREAKPOINT)
    ...
    #                           ])
    debugger.set_breakpoints(Breakpoints.breakpoints())
    #debugger.go_to_next_breakpoint()
...
def on_drag_and_drop():
    debugger.go_to_next_breakpoint(Breakpoints.LANDING_POINT_DRAG_DROP_START)
    debugger.go_to_next_breakpoint(Breakpoints.LANDING_POINT_DRAG_DROP_PATH)
    path_pointer = debugger.get_register("ECX")
    path_length = debugger.get_register("EAX")
    context.path = debugger.read(path_pointer, path_length).decode('ascii')
    print("Apparently, we are processing file: {}".format(context.path))

    assert context.path[-4:].lower() == '.map'

    context.pngpath = context.path[:-4] + ".png"

    debugger.go_to_next_breakpoint(Breakpoints.LANDING_POINT_DRAG_DROP_PNGPATH)
    _pngpath = debugger.read(debugger.get_register("ECX"), path_length).decode('ascii')
    _mappath = debugger.read(debugger.get_register("EDX"), path_length).decode('ascii')

    assert context.pngpath == _pngpath
    assert context.path == _mappath

def convert_file():
    #mappath, pngpath
    debugger.go_to_next_breakpoint(Breakpoints.CONVERT_FILE_START)
    context.path = _make_path_local(context.path)
    context.mapfilehandle = open(context.path, 'rb')

    assert b'\xFF\xFF\xFF\xFF' == context.mapfilehandle.read(4)
    context.pngpath = _make_path_local(context.pngpath)
    context.pngfilehandle = open(context.pngpath, 'wb')

def convert_and_write():
    #mapfilepointers, pngfilehandle
    debugger.go_to_next_breakpoint(Breakpoints.CONVERT_AND_WRITE_START)
    return

    number = struct.unpack("<I", context.mapfilehandle.read(4))[0]
    D.assert_equal(number, 0x27F5)

    number = struct.unpack("<I", context.mapfilehandle.read(4))[0]
    D.assert_equal(number, 0x9E40)

    number = struct.unpack("<I", context.mapfilehandle.read(4))[0]
    D.assert_equal(number, 0x27E9)

    number = struct.unpack("<I", context.mapfilehandle.read(4))[0]
    D.assert_equal(number, 0xA12E9770)

def file_processor():
    #cb2_read_chunk_of_size, callback1, myst2, mapfilehandle
    debugger.go_to_next_breakpoint(Breakpoints.FILE_PROCESSOR_START)

    a1.data = context.mapfilehandle.read(a1.buffer_size)

from pnger_constants import data_0x41d1d8

def sub_402770(eax, esi, ebp, ebx):
    debugger.go_to_next_breakpoint(Breakpoints.SUB_402770_START)
    #assert_equal(esi, struct.unpack("<I", debugger.read(debugger.get_register("ESP") + 0x8, 4))[0])
    #assert_equal(eax, struct.unpack("<I", debugger.read(debugger.get_register("ESP") + 0x4, 4))[0])
    #assert_equal(ebp, struct.unpack("<I", debugger.read(debugger.get_register("ESP") + 0xB, 4))[0])
    ecx = struct.unpack("<I", debugger.read(debugger.get_register("ESP")+0x8, 4))[0]
    if ecx == 0:
        return 0
    eax = struct.unpack("<I", debugger.read(debugger.get_register("ESP")+0x4, 4))[0]
    esi = struct.unpack("<I", debugger.read(debugger.get_register("ESP")+0xC, 4))[0]
    print(ecx)
    print(eax) #this is the result from the previous call to sub_402770
    print(esi)
    eax = eax ^ 0xFFFFFFFF
    if esi >= 8:
        #_push_edi = edi
        ...
        while True:
            ebx = 0
            ebx = struct.unpack("<B", testdat_memory.get(ecx, 1))[0]
            edx = eax
            edx &= 0xFF
            esi -= 8
            edx ^= ebx
            ebx = eax
            eax = struct.unpack("<I", data_0x41d1d8.get(edx*4, 4))[0]
            ebx >>= 8
            eax ^= ebx
            ebx ^= ebx
            ebx = struct.unpack("<B", testdat_memory.get(ecx+1, 1))[0]
            edx = eax
            edx &= 0xFF
            eax >>= 8
            ecx += 1
            edx ^= ebx
            edx = struct.unpack("<I", data_0x41d1d8.get(edx * 4, 4))[0]
            edx ^=  eax
            ebx ^= ebx
            ebx = struct.unpack("<B", testdat_memory.get(ecx+1, 1))[0]
            eax = edx
            ...
            ecx += 1
            eax ^= ebx
            eax = struct.unpack("<I", data_0x41d1d8.get(eax * 4, 4))[0]
            eax ^= edx
            ebx ^= ebx
            ebex = struct.unpack("<B", testdat_memory.get(ecx+1, 1))[0]
            edx = eax
            edx &= 0xFF
            eax >>= 8
            ecx += 1
        ...
        if esi != 0:
            while True:
                ...
                if esi == 0:
                    break
        eax = eax ^ 0xFFFFFFFF
        debugger.go_to_next_breakpoint(Breakpoints.SUB_402770_END)
        return eax
...
testdat_memory = Memory()
testdat_memory.set(0x19D73A, testdat)
testdat_pointer = MemoryPointer(testdat_memory, 0x19D73A)

with open("resources/MxM_unseen_1.png", 'rb') as f:
    pngfile = f.read()

heap_allocation_1 = Memory()
heap_allocation_1_pointer = MemoryPointer(heap_allocation_1, 0)

def callback1():
    while debugger.get_breakpoint() != Breakpoints.CALLBACK1_START:
        debugger.go_to_next_breakpoint()
    debugger.go_to_next_breakpoint(Breakpoints.CALLBACK1_INIT)
    ebx = struct.unpack("<I", debugger.read(debugger.get_register("ESP") + 0xC, 4))[0]
    ebp = context.mapfilehandle
    esi = struct.unpack("<I", debugger.read(debugger.get_register("ESP") + 0x10, 4))[0]
    eax = struct.unpack("<I", debugger.read(debugger.get_register("EBP") + 0xC, 4))[0]

    edi = debugger.get_register("EDI")
    sub_402770(eax, esi, ebp, edi)

    edi = struct.unpack("<I", debugger.read(debugger.get_register("EBP") + 0x8, 4))[0]
    heap_allocation_1_pointer.pointer = edi

    assert ebx == 0x1000

    h1p = heap_allocation_1_pointer + 0
    tdp = testdat_pointer + 0

    for i in range(ebx >> 2):
        h1p.dword = tdp.dword
        h1p += 4
        tdp += 4

    for i in range(ebx & 0x3):
        h1p.byte = tdp.byte
        h1p += 1
        tdp += 1
...
Breakpoints.CALLBACK1_START = ("0x402210", "callback1_start")
Breakpoints.CALLBACK1_INIT = ("0x40222C", "callback1_init")
Breakpoints.SUB_402770_START = ("0x402770", "sub_402770_start")
Breakpoints.SUB_402770_END = ("0x4028A4", "sub_402770_end")
